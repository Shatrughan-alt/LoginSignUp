...
routes.post("/register", async (req, res) => {
    ...
    const alreadyuser = await todoschema.findOne({ email })
    ...
    if (alreadyuser) {
        ...
        const newUser = new todoschema({ email, password: securePass })
        newUser.save();
...
routes.post("/login", async (req, res) => {
    ...
    try {
        const userPresent = await todoschema.findOne({ email })
        if (!userPresent) {
